            ...
            if (data) {
                const e = await map(data.content,
                  async   (w, i) => 
                        ...
                        `${i + 1} [${( (await client.users.fetch(w.botid)).username)}](https://discord.com/api/oauth2/authorize?client_id=` + (w.botid) + `&permissions=8&scope=bot%20applications.commands) :  ${(Math.round((new Date(w.time).getTime() - new Date(nowtime).getTime()) /(1000*3600*24))) >=1 ? `<t:${(Math.round((new Date(w.time).getTime()) /1000))}:R>` : "Expiré"} `
                )
